...
# -*- coding: utf-8 -*-
"""
etl.py — Part 2 only (keep separate from load_to_mariadb.py)

Runs ETL checks & exception reporting exactly as required:
  Department_Information
    - Report: any missing value
    - Report: Department_ID duplicates
    - Report: Department_Name duplicates
    - Report: DOE < 1900 or non-integer
  Employee_Information
    - None required (emit empty exceptions CSV)
  Student_Counceling_Information
    - Report: Department_Admission missing
    - Report: Department_Admission not found in Department_Information.Department_ID
  Student_Performance_Data
    - Report & DISCARD: any row with missing values
    - Report & DISCARD: Marks not in [0,100] or non-numeric
    - Report & DISCARD: Effort_Hours not integer or < 0
    - Report: (Student_ID, Paper_ID) duplicate pairs
    - Output cleaned table: Student_Performance_Data_Clean
...
def run_part2_etl(engine, out_dir: str = "etl_outputs"):
    ...
    stud_ex = []
    if not stud.empty:
        # Department_Admission missing
        if "Department_Admission" in stud.columns:
            ...
            # Department_Admission must exist in Department_Information.Department_ID
            ...
            for r, v in stud["Department_Admission"].items():
                ...
                if s and dept_ids and s not in dept_ids:
                    stud_ex.append({"row_index": int(r),
                                    ...
                                    "details": f"Department_Admission={s} not found in Department_Information.Department_ID"})
    ...
    discard_idx = set()
    if not perf.empty:
        # Any missing values -> discard & report
        ...
        # Marks in [0,100] (numeric) -> discard & report
        ...
        # Effort_Hours integer >= 0 -> discard & report
        if "Effort_Hours" in perf.columns:
            for r, v in perf["Effort_Hours"].items():
                try:
                    i = int(v)
                    # If original is float but not whole, reject; also reject negatives
        ...
        # (Student_ID, Paper_ID) unique -> report (do not discard automatically)
    ...
    # Upsert cleaned table back into DB
    with engine.begin() as con:
